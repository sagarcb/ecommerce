...
@section('pageTitle') <a href="{{route('shipping.method.add')}}">Add Shipping Method</a> @endsection
...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('shipping.method.store') }}" id="myform" enctype="multipart/form-data">
                @csrf
                <div class="form-row col-md-10">
                    <label for="description">Shipping Method Name</label>
                    <input type="text" id="name" class="form-control" name="name" placeholder="Write shipping method name" required>
                    <font color="red">{{ ($errors->has('name'))?($errors->first('name')): '' }}</font>
                </div>

                <div class="form-row col-md-10">
                    <label for="description">Cost</label>
                    <input type="number" id="cost" class="form-control" name="cost" placeholder="Write shipping cost" required>
